...
//								// so next bower can take a throw
			...
			if(gamePlay.isPartyAssigned() && !gamePlay.isGameFinished()){
				...
				if (gamePlay.bowlerIterator.hasNext()) {
					...
					while (canThrowAgain) {
						...
						//canThrow = true;
						if (canThrow) {
							System.out.println("allowed to throw  : " + canThrow);
	...
	public void setCanThrow( int Angle, int Velocity) {
		System.out.println("inside setCanThrow() func...");
		canThrow = true;
		System.out.println(canThrow);
		angle = Angle;
		velocity = Velocity;
	}
